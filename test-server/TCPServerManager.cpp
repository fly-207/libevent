...
bool CTCPServerManager::Start()
{
    ...
    for (auto& _item : m_workBaseVec)
    {
        ...
        _item.accept_notify[0] =
			event_new(_item.base, pair[0], EV_WRITE, callback1, &_item);
		_item.accept_notify[1] = event_new(
			_item.base, pair[1], EV_READ, callback1, NULL);

 
        {
            auto v = evtimer_new(_item.base, time_cb, event_self_cbarg());
            struct timeval tv;
            tv.tv_sec = 0;
        ...
        auto v = &_item.base->nactivequeues;
        printf("工作线程-%d,base地址-%p,监控地址=%p\n", index++, _item.base, v);
    }
...
void CTCPServerManager::ThreadAccept(CTCPServerManager* pThis)
{
    ...
    {
        auto v = evtimer_new(pThis->m_listenThreadInfo.base, time_cb, event_self_cbarg());
        struct timeval tv;
        tv.tv_sec = 0;
        tv.tv_usec = rand_int(50000);
        evtimer_add(v, &tv);
...
void CTCPServerManager::ThreadSendMsg(CTCPServerManager* pThreadData)
{
    ...
    {
        int index = 0;
        for (auto& _item : pThreadData->m_workBaseVec)
        {
            _item.base = event_base_new();
            printf("工作线程-%d,base地址-%p,优先级%d\n", index++, _item.base, _item.base?_item.base->nactivequeues:0);
        }
        Sleep(100);
    }
